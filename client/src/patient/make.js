...
useEffect(() => {
   ...
   var day1=dateAppointment[0]; 
   var  day2=dateAppointment[2]; 
           //1 2 4
 ...
 <Link to="/patM"><MDBBtn color="indigo">Back</MDBBtn></Link>
  ...
  <MDBRow md="4">
    ...
    <p className="h5 text-center mb-4">Enter Appointment Date.</p>
        ...
        <p className="h5 text-center mb-4">Enter Your ID.</p>
        <div className="blue-text">
          <MDBInput label="Type your ID" icon="envelope" onChange={(e) => {setID(e.target.value)}}/>
        </div>
        <div className="text-center">
          <MDBBtn color="indigo" href={`http://localhost:3000/bookTime?id=${IdFromURL}&date=${sendDate}&pid=${pid}`}>Submit</MDBBtn>
        </div>
      </form>
        </MDBCardBody>
        </MDBCard>
